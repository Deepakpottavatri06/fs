...
public class Precursor{
    public static void main(String [] args){
        ...
        for(int i = 0; i < k ; i++){
            query[i][0] = cin.nextInt();
            query[i][1] = cin.nextInt();
        }
        
        System.out.println(solveQuery(n,m,k,deps,query));
        cin.close();
    }
    static String solveQuery(int n , int m , int k , int deps[][], int query[][]){
        ...
        for(int i = 0; i < k ; i++){
            solved[i] = helper(adjList,query[i][0],query[i][1]);
        }
        
        return Arrays.toString(solved);
    }
    static boolean helper(List<List<Integer>> adjList , int s, int t){
        if(adjList.get(s).contains(t)) return true;
        
        for(int i : adjList.get(s)){
            if(helper(adjList,i,t)) return true;
        }
        return false;
    }
}