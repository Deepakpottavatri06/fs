...
He is trying to fScannerrm a palindrome using two strings P and Q.
...
public class Palindrome{
    ...
    static boolean find(String p, String q){
        int n = p.length();
        for(int i = 0; i < n ; i++){
            if(check(p.substring(0,i)+q.substring(i)) || check(q.substring(0,i)+p.substring(i))){
                return true;
            }
        }
        return false;
