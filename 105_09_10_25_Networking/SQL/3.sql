...
use mdb;
with directors_reviewer as (
    select d.Artist_id from Directions d inner join Reviews r on d.Artist_id=r.Artist_id
)
select a.name as artist_name from Artists a where a.id in (select Artist_id from directors_reviewer) ;
