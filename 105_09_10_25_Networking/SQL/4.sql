...
with pre1980 as (
    select avg(r.rating) as pre1980_avg from Reviews r join Movies m on r.Movie_id=m.id
    where m.release_year < 1980
)
,
post1980 as (
    select avg(r.rating) as post1980_avg from Reviews r join Movies m on r.Movie_id=m.id
    where m.release_year >= 1980
)

select pr.pre1980_avg , po.post1980_avg , round(pr.pre1980_avg-po.post1980_avg,2) as rating_difference from 
pre1980 pr , post1980 po;