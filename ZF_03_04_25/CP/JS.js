...
class CustomMap {
    ...
    set(key,value){
        ...
        this.items[serializedKey] = value;
    }
    get(key){
        const serializedKey = this._serialize(key);
        return this.items.hasOwnProperty(serializedKey)? this.items[serializedKey]: undefined;
    }
    has(key){
        const serializedKey = this._serialize(key);
        return this.items.hasOwnProperty(serializedKey);
    }
    deleteKey(key){
        const serializedKey = this._serialize(key);
        if(this.items.hasOwnProperty(serializedKey)){
            delete this.items[serializedKey];
            return true;
        }
        return false;
    }
