...
use mdb;
with Ratings as (
    select a.name,round(avg(r.rating),2) as avg_director_rating from Artists a join Directions d 
    on a.id = d.Artist_id join Reviews r on d.Movie_id = r.Movie_id group by a.id,a.name
    )
, avgratings as (
    select round(avg(rating),2) as global_avg from Reviews
    )

select r.name as director_name,r.avg_director_rating,av.global_avg from Ratings r CROSS JOIN avgratings av
where avg_director_rating> av.global_avg order by avg_director_rating desc;
