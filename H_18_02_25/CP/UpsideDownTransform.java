...
public class UpsideDownTransform {
    public TreeNode upsideDownBinaryTree(TreeNode root) {
        //Write your logic here
        return construct(root);
	}
	static TreeNode construct(TreeNode root){
        if(root.left==null && root.right==null){
	        return root;
	    }
	    TreeNode left = construct(root.left);
	    TreeNode rightOfLeft = rightMost(left);
        rightOfLeft.left = root.right;
	    rightOfLeft.right = root;
	    root.left = null;
	    root.right = null;
	    return left;
	}
