...
public class PandaConverter{
    public static void main (String[] args) {
        try (Scanner cin = new Scanner(System.in)) {
            String word1 = cin.next();
            String word2 = cin.next();
            int s = cin.nextInt();
            System.out.println(find(word1,word2,s));
        }
    }
    static boolean find(String word1, String word2, int s){
        ...
        for(int i = 0; i < word1.length(); i++){
            ...
            if(a!=b){
                int p = (26 + (int)(b-a)) % 26;
                // System.out.println("p steps : "+p);
                int totalP = p + used[p]*26;
                if((s<totalP)){
                    return false;
                }
                used[p]++;
