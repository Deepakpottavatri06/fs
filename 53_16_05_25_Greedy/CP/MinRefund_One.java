...
public class MinRefund_One{
    public static void main (String[] args) {
        try (Scanner cin = new Scanner(System.in)) {
            ...
            System.out.println(refund(inp,arr));
        }
    }
    static int refund(String inp, int arr[]){
        int total = 0;
        for(int i:arr){
            total+=i;
        }
        ...
        for(int i = 1; i < arr.length ; i++){
            if(curr == inp.charAt(i)){
                max = Math.max(max,arr[i]);
            }
            else{
                // System.out.println("The max for char "+curr+" is : "+max);
                currSum += max;
                max = arr[i];
                curr = inp.charAt(i);
            }
