...
h_fence = [0]+ list(map(int,input().split())) + [l]
v_fence = [0]+ list(map(int,input().split())) + [w]

h_fence.sort()
v_fence.sort()
max_h_diff = max([h_fence[i+1] - h_fence[i] for i in range(h+1)])
max_v_diff = max([v_fence[i+1] - v_fence[i] for i in range(v+1)])
print(max_v_diff*max_h_diff)
