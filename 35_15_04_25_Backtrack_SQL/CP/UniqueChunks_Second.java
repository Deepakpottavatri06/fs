...
public class UniqueChunks_Second{
    ...
    static int find(String inp){
        ...
        btrack(max,inp,0,set);
        return max[0];
    }
    static void btrack(int max[], String inp, int ind, Set<String> set){
        if(ind==inp.length()){
            max[0] = Math.max(max[0],set.size());
            return;
        }
        for(int i = ind; i < inp.length() ; i++){
            String word = inp.substring(ind,i+1);
            if(set.contains(word)) continue;
            set.add(word);
            btrack(max,inp,i+1,set);
            set.remove(word);
        }
    }
