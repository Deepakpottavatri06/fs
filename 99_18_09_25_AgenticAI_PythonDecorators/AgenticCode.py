...
=========================================================
This is a teaching skeleton: it shows control flow, typed tools, budgets, and safe prompts. 
Replace call_llm() with your model API. Note we never ask the model to reveal chain‑of‑thought, 
only structured JSON or final bullets
...
def call_llm(prompt: str) -> str:
    ...
    return alt.text.strip() # type: ignore
...
class ResearchBriefAgent:
    ...
    def run(self, topic: str) -> Tuple[str, str]:
        ...
        pathlib.Path("out/trace.jsonl").write_text(
            "\n".join(json.dumps(vars(e),indent=2) for e in self.trace), encoding="utf-8"
        )
...
if __name__ == "__main__":
    # Small fake document index
    sample_index = [
        Chunk(doc_id="D1", title="Deep Learning Intro", year=2021, page=1,
              text="MobileNetV2 improves efficiency with depthwise separable convolutions.",
              cite_label="Smith 2021, p.1"),
        Chunk(doc_id="D2", title="CNN Advances", year=2022, page=2,
              text="Residual connections improve gradient flow in deep neural networks.",
              cite_label="Lee 2022, p.2"),
    ]

    agent = ResearchBriefAgent(
        docsearch=DocSearch(sample_index),
        save=SaveOutput(),
        budgets=Budgets(max_tool_calls=8, max_time_s=30)
    )

    brief_path, sources_path = agent.run("MobileNetV2 improvements")
    print("✅ Brief saved at:", brief_path)
    print("✅ Sources saved at:", sources_path)
