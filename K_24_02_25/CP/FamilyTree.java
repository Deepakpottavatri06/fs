...
public class FamilyTree {
    public static void main(String[] args) {
        ...
        System.out.print(fTree(root));
        cin.close();
    }
    static String fTree(Node root){
        StringBuilder stringTree = new StringBuilder();
        preOrder(root,stringTree);
        return stringTree.toString();
    }
    static void preOrder(Node root, StringBuilder stringTree){
        if(root==null){
            return;
        }
        if(root.left==null && root.right==null){
            stringTree.append(root.val);
            return;
        }
        stringTree.append(root.val);
        stringTree.append("(");
        preOrder(root.left, stringTree);
        stringTree.append(")");
        if(root.right==null){
            return;
        }
        stringTree.append("(");
        preOrder(root.right, stringTree);
        stringTree.append(")");
    }
