...
for i in range(n):
    ...
    interviews.append((s,e,i))
interviews.sort()
meet_links = [i for i in range(n)]
consumed = []
heapq.heapify(meet_links)
for s,e,i in interviews:
    while len(consumed)!=0 and consumed[0][0]<=s:
        heapq.heappush(meet_links,consumed[0][1])
        heapq.heappop(consumed)
    smallest_meet_link = heapq.heappop(meet_links)
    if i==c:
        print(smallest_meet_link)
        break
    heapq.heappush(consumed,(e,smallest_meet_link))
