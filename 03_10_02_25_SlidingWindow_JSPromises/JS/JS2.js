...
function fetchAvailableMovies() {
  ...
  function selectShowtime(movie) {
    return new Promise((resolve) => {
      console.log(`Selecting showtime for Movie ${movie}...`);
      // Write your logic here
      setTimeout(()=>{
          resolve(`Showtime for Movie ${movie} selected.`);
      },1500);
  ...
  async function processBooking(userId) {
    ...
    // First Concurrent Activities: Fetch available movies and check user details
      const [fet,check] = await Promise.all([fetchAvailableMovies(),checkUserDetails(userId)]);
      console.log(check);
    // Sequential Activities: Choose movie and select showtime
      const movie = await chooseMovie(fet);
      const select = await selectShowtime(movie);
      console.log(select);
      const pay  =await makePayment();
      console.log(pay);
    // Concurrent Activities: Send email and SMS confirmations
    const [email,sms] = await Promise.all([sendEmailConfirmation(),sendSMSConfirmation()]);
    console.log(email);
    console.log(sms);
