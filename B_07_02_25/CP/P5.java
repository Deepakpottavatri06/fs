...
public class P5{
    public static void main(String [] args){
        ...
        System.out.println(solve(n,k,x,f,t));
        cin.close();
    }
    static List<Integer> solve(int n, int k , int x, int f, int t []){
        Map<Integer, Integer> map = new HashMap<>();
        List<Integer> res = new ArrayList<>();
        for(int i = 0; i < k ; i++){
            map.put(t[i],map.getOrDefault(t[i],0)+1);
        }
        int value = findValue(map,x,f);
        res.add(value);
        int ind = 0;
        for(int  i = k; i < n ; i++){
            map.put(t[i],map.getOrDefault(t[i],0)+1);
            map.put(t[ind],map.get(t[ind])-1);
            if(map.get(t[ind])==0){
                map.remove(t[ind]);
            }
            ind++;
            value = findValue(map,x,f);
            res.add(value);
        }
        return res;
    }
    static int findValue(Map<Integer, Integer> map , int x, int f){
        ...
        for( Map.Entry<Integer, Integer> e : map.entrySet()){
            int pow = (int)Math.pow(e.getKey(),f);
            pq.add(new int []{e.getKey(),e.getValue()*pow});
        }
        int res = 0;
        while(x>0 && !pq.isEmpty()){
            int temp [] = pq.poll();
            res += temp[0]*map.get(temp[0]);
            x--;
        }
        return res;
