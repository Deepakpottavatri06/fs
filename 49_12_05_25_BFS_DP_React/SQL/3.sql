...
select p.name as ProductName,sum(oi.Quantity) as TotalQuantitySold from 
Products p join OrderItems oi on p.ProductID  = oi.ProductID
group by p.ProductID having sum(oi.Quantity) >= 2;
