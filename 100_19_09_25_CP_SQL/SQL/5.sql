...
select p.category , sum((select sum(s.qty) from stock_movements s where p.product_id=product_id)) as on_hand
from products p group by p.category having on_hand<500 ;