...
WITH prod_last AS (
    SELECT
        ...
        p.sku,
        p.category,
        MAX(s.movement_date) AS last_mv
    FROM products p
    JOIN stock_movements s USING (product_id)
...
FROM prod_last pl
WHERE pl.last_mv <= (
    -- minimum last_mv among the other products in the same category
    SELECT MIN(other.last_mv)
    FROM prod_last other
