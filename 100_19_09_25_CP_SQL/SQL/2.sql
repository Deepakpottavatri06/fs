...
with 
inv_value_table as 
(select warehouse_id,sum(qty*unit_cost) as inv_value from stock_movements group by warehouse_id) 

select w.warehouse_id, w.name, i.inv_value from warehouses w join inv_value_table i using(warehouse_id)
where i.inv_value > (select avg(inv_value) from inv_value_table) ;
