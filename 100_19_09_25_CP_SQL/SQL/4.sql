...
with sku_table as (
    select p.sku ,sum(s.qty) as total_qty, p.product_id from products p join stock_movements s using(product_id)  group by p.sku
)

select w.warehouse_id, w.name, p.sku, sum(s.qty) as wh_qty , sk.total_qty
from warehouses w join stock_movements s on w.warehouse_id = s.warehouse_id
join products p on p.product_id=s.product_id 
join sku_table sk on p.product_id=sk.product_id
GROUP BY w.warehouse_id, w.name, p.sku, sk.total_qty
HAVING SUM(s.qty) >= 0.4 * sk.total_qty
