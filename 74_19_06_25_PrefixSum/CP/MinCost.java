...
public class MinCost{
    public static void main (String[] args) {
        ...
        int ha = cin.nextInt(), hb = cin.nextInt(), hc = cin.nextInt(), ab = cin.nextInt(), bc = cin.nextInt(), ca = cin.nextInt();
        int min = Integer.MAX_VALUE;
        min = Math.min(min,ha+ca+hc);
        min = Math.min(min,hb+bc+hc);
        min = Math.min(min,ha+ca+bc+hb);
        min = Math.min(min,hb+bc+ca+ha);
        min = Math.min(min,hc+ca+ab+hb);
        min = Math.min(min,hb+ab+ha);
         min = Math.min(min,hc+ca+ha);
          
        min = Math.min(min,2*ha+2*hc);
        min = Math.min(min,2*hb + 2*ha);
        min = Math.min(min,2*hc + 2*ha); 
