...
select a.name as director_name, count(distinct r.Artist_id) as unique_reviewers
from Artists a join Directions d on a.id = d.Artist_id
join Reviews r on d.Movie_id = r.Movie_id
group by a.name
order by unique_reviewers desc;
-- where r.Artist_id!=d.Artist_id and d.Movie_id = r.Movie_id;
