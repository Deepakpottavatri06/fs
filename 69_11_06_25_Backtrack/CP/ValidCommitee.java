/*A group of students is forming a tech club, and they are 
interviewing K × K students. Each student has two skill levels:
 - Mathematics (M)
 - Physics (P)
These skill levels range from 0 to K-1.

They want to form committees of size N under the following 
conditions:
 - All members of a committee must have different Mathematics 
   skill levels.
 - All members must have different Physics skill levels.
 - For any two students in the committee, the difference of their 
   Math skills must not equal the difference of their Physics skills, 
   i.e., |M1 - M2| != |P1 - P2|

Input Format:
-------------
input1: Integer N – desired committee size
input2: Integer K – number of skill levels for Math and Physics 
(total students = K × K)

Output Fromat:
--------------
An integer representing the number of valid committees of size N 
that can be formed from the K × K students. Return the result 
modulo 10⁹ + 7.

Sample Input: 
-------------
2
3

Sample Output: 
--------------
8

Explanation:
------------
Examples of some valid pairs:
(0,0) and (1,2) → |0-1| = 1, |0-2| = 2 ✅
(0,1) and (1,0) → |0-1| = 1, |1-0| = 1 ❌ (same diff → invalid)
(0,0) and (2,1) → |0-2| = 2, |0-1| = 1 ✅

8 Valid Pairs: 1. (0,0) - (1,2)
2. (0,0) - (2,1)
3. (0,1) - (2,0)
4. (0,2) - (1,0)
5. (0,2) - (2,1)
6. (1,0) - (2,2)
7. (1,2) - (0,0)
8. (1,2) - (2,0)

Sample Input: 
-------------
2
2

Sample Output: 
--------------
0

 */
import java.util.Scanner;

public class ValidCommitee {
    public static void main (String[] args) {
        try (Scanner cin = new Scanner(System.in)) {
            int n = cin.nextInt();
            int k = cin.nextInt();
            System.out.println(find(n,k));
        }
    }
    static int find(int n, int k){
        int grid [][] = new int[k][k];
        int res []= new int[]{0};
        backtrack(0,n,k,grid,res);
        return res[0];
    }
    static void backtrack( int c,int n, int k , int grid[][], int res[]){
        if(n==0){
            res[0]++;
            res[0]%=1_000_000_007;
            return;
        }
        if(c>=k){
            return;
        }
        for(int i = 0; i < k; i++){
            if(isSafe(grid,i,c)){
                grid[i][c] = 1;
                backtrack(c+1,n-1,k,grid,res);
                grid[i][c] = 0;
            }
        }
        
        backtrack(c+1,n,k,grid,res);
    }
    static boolean isSafe(int grid[][], int r, int c){
        int k = grid.length;
        
        for(int i = c-1; i >= 0; i--){
            if(grid[r][i]==1){
                return false;
            }
        }
        
        for(int i = r-1 , j = c-1; i>=0 && j>=0 ; i--,j--){
            if(grid[i][j]==1){
                return false;
            }
        }
        for(int i = r +1, j = c-1; i<k && j>=0 ; i++,j--){
            if(grid[i][j]==1){
                return false;
            }
        }
        return true;
    }
}